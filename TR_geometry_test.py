...
av_length=round(np.mean([len(unit) for unit in units_coords]))
smoothed_structure=[np.mean(region[i:i+av_length],axis=0) for i in range(len(region)-av_length+1)]


#Breakpoints gives, for each unit, its bounds in the smoothed structure
breakpoints=np.asarray(np.cumsum([len(unit) for unit in units_coords]))  
breakpoints -= av_length
breakpoints[0]=0
breakpoints[-1]=len(smoothed_structure)

Nsmooth=len(smoothed_structure)
...
#CURVATURE
def rotation_fit(C=smoothed_structure,breakpoints=breakpoints,window=10):
    data=np.asarray(C)
    index_list=[]
    rotation_list=[]
    centers_list=[]
    for i in range(max(N-window+1,1)):
        ...
        start=breakpoints[indexes[0]]
        end=breakpoints[indexes[-1]]
        data_to_fit=C[start:end]
    ...
    return av_centers,centers_list

if N>=3:
    rot_centers,cand_centers=rotation_fit()
...
if args.draw:
    ...
    for i in range(Nsmooth):
        cmd.pseudoatom('smoothed_structure',pos=tuple(smoothed_structure[i]))
    ...
    for i in range(len(cand_centers)):
        cmd.pseudoatom('cand_centers',pos=tuple(cand_centers[i]))
