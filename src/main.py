...
def main():
    ...
    # Initialize GeomeTRe with optional PyMOL drawing
    geometre = GeomeTRe(draw_enabled=args.draw if 'draw' in args else False)

    if args.mode == 'single':
        result_path = geometre.compute(
            mode="single",
        ...
        logger.info(f"Running batch mode with arguments: {args.batch}, {args.output}, {args.format}, {args.threads}, {args.pdb_dir}")
        result_path = geometre.compute(
            mode="batch",
            batch=args.batch,
            output=args.output,
